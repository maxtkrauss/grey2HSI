...
from src.models.unet_model import Generator as Generator
...
class Pix2Pix(pl.LightningModule):
    def __init__(self, run):
        ...
        # Models
        self.discriminator = Discriminator(in_channels_x=c.SHAPE_X[0], in_channels_y = 1).to(c.DEVICE)
        match c.GENERATOR_MODEL:
            ...
            case "spectral_unet":
                self.generator = spectral_Unet(in_channels=c.SHAPE_X[0], out_channels=c.SHAPE_Y[0], features=[64, 128, 256, 512, 1024]).to(c.DEVICE)
            case "SRNET":
                self.generator = SRNET(in_channels=c.SHAPE_X[0], out_channels=c.SHAPE_Y[0])
            case _:
    ...
    def generator_loss(self, prediction_image, target_image, prediction_label, target_label, real_label):
        ...
        """
        # Adverserial loss
        BCE_loss = self.BCE(prediction_label, torch.ones_like(target_label))
        # LFM loss
        ...
        L2 = torch.sum((prediction_image - target_image) ** 2)
        ...
        MSE = u.MSE(prediction_image, target_image)
        MS_SSIM = u.multiscale_ssim(prediction_image, target_image)
        return BCE_loss, L1, L2, SPEC, RASE, SSIM, MSE, MS_SSIM
    ...
    def training_step(self, batch, batch_idx):
        ...
        gen_bce_loss, gen_l1_loss, gen_l2_loss, gen_spec_loss, gen_rase_loss, gen_ssim, gen_mse, gen_ms_ssim = gen_loss_tuple
        generator_loss = gen_bce_loss * c.LAMBDA_ADV 
        generator_loss += gen_l1_loss * c.LAMBDA_L1
        generator_loss += gen_l2_loss * c.LAMBDA_L2
        ...
        generator_loss += (1- gen_ssim) * c.LAMBDA_SSIM 
        generator_loss += gen_mse * c.LAMBDA_MSE
        generator_loss += gen_ms_ssim * c.LAMBDA_MS_SSIM
        # Generator Optimizer
    ...
    def on_validation_epoch_end(self):
        ...
        self.log('ssim3D', metrics['val/gen/ssim3D'])
        # Log to neptune
